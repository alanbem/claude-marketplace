...
if [ "$IS_GIT" -eq 0 ]; then
    ...
    # Per-value dimming: zero values use very dim, non-zero use normal dim
    FA="${COLOR_GREEN_DIM}"; [ "$GIT_FILES_ADDED" -eq 0 ] && FA="${COLOR_GREEN_DIM_VERY}"
    FM="${COLOR_ORANGE_DIM}"; [ "$GIT_FILES_MODIFIED" -eq 0 ] && FM="${COLOR_ORANGE_DIM_VERY}"
    FD="${COLOR_RED_DIM}"; [ "$GIT_FILES_DELETED" -eq 0 ] && FD="${COLOR_RED_DIM_VERY}"
    FU="${COLOR_WHITE_DIM}"; [ "$GIT_FILES_UNTRACKED" -eq 0 ] && FU="${COLOR_WHITE_DIM_VERY}"
    # Dim icon when all zeros
    ...
    FILES_VALS="${FA}+${GIT_FILES_ADDED}${RST} ${FM}~${GIT_FILES_MODIFIED}${RST} ${FD}−${GIT_FILES_DELETED}${RST} ${FU}?${GIT_FILES_UNTRACKED}${RST}"

    LA="${COLOR_GREEN_DIM}"; [ "$GIT_LINES_ADDED" -eq 0 ] && LA="${COLOR_GREEN_DIM_VERY}"
    LR="${COLOR_RED_DIM}"; [ "$GIT_LINES_REMOVED" -eq 0 ] && LR="${COLOR_RED_DIM_VERY}"
    ...
    DIFF_VALS="${LA}+${GIT_LINES_ADDED}${RST} ${LR}−${GIT_LINES_REMOVED}${RST}"
...
# Per-value dimming for session stats
SA="${COLOR_GREEN_DIM}"; [ "$SESSION_LINES_ADDED" -eq 0 ] && SA="${COLOR_GREEN_DIM_VERY}"
SR="${COLOR_RED_DIM}"; [ "$SESSION_LINES_REMOVED" -eq 0 ] && SR="${COLOR_RED_DIM_VERY}"
...
STATS_VALS="${SA}✚${SESSION_LINES_ADDED}${RST} ${SR}−${SESSION_LINES_REMOVED}${RST}"
